...
//        request.debug(repoSys);
  ...
  private function getSession(offline=false) {
...
//        request.debug(iTrace.toString());
  ...
  public function pom(required artifactId, pomFile = getTempDirectory() & "pom.xml") {
    var msession = getSession();
    var artifact = jDefaultArtifact.init( artifactId );
    ...
    var writer = cl.create("java.io.FileWriter").init(pomFile);
